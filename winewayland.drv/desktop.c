/*
 * WAYLANDDRV desktop window handling
 *
...
static struct waylanddrv_mode_info *dd_modes;
...
static void make_modes(void)
{
    ...
    for (i=0; i<ARRAY_SIZE(screen_sizes); i++)
    {
        if ( (screen_sizes[i].width <= max_width) && (screen_sizes[i].height <= max_height) )
        {
            ...
            {
                // only add them if they are smaller than the root window and uniqu
    ...
    if ((max_width != screen_width) && (max_height != screen_height))
    {
        ...
        WAYLANDDRV_Settings_AddOneMode(max_width, max_height, 0, 60);
    }
}

static int WAYLANDDRV_desktop_GetCurrentMode(void)
{
    return 0;
    unsigned int i;
...
static LONG WAYLANDDRV_desktop_SetCurrentMode(int mode)
{
    DWORD dwBpp = screen_bpp;
    ...
    //WAYLANDDRV_resize_desktop(dd_modes[mode].width, dd_modes[mode].height);
    return DISP_CHANGE_SUCCESSFUL;
}

/***********************************************************************
 *		WAYLANDDRV_init_desktop
 *
 * Setup the desktop when not using the root window.
 */


void WAYLANDDRV_init_desktop( unsigned int width, unsigned int height )
{
    RECT primary_rect = get_primary_monitor_rect();
    ...
    dd_modes = WAYLANDDRV_Settings_SetHandlers("desktop", 
                                           WAYLANDDRV_desktop_GetCurrentMode, 
                                           WAYLANDDRV_desktop_SetCurrentMode, 
                                           ARRAY_SIZE(screen_sizes)+2, 1);
    make_modes();
    WAYLANDDRV_Settings_AddDepthModes();
    dd_mode_count = WAYLANDDRV_Settings_GetModeCount();
}


/***********************************************************************
 *		WAYLANDDRV_create_desktop
 *
 * Create the X11 desktop window for the desktop mode.
 */
BOOL CDECL WAYLANDDRV_create_desktop( UINT width, UINT height )
{
    ...
    // Create window 
    //win_attr.event_mask = ExposureMask | KeyPressMask | KeyReleaseMask | EnterWindowMask | PointerMotionMask | ButtonPressMask | ButtonReleaseMask | FocusChangeMask;
    ...
    //WAYLANDDRV_init_desktop( win, width, height );
    WAYLANDDRV_init_desktop( width, height );
    return TRUE;
    */
}
...
static BOOL CALLBACK update_windows_on_desktop_resize( HWND hwnd, LPARAM lparam )
{
    ...
    {
        POINT pos = virtual_screen_to_root( data->whole_rect.left, data->whole_rect.top );data->vis.screen, mask, &changes );
    }
...
/***********************************************************************
 *		WAYLANDDRV_resize_desktop
 // TODO check what this does
 */
void WAYLANDDRV_resize_desktop( unsigned int width, unsigned int height )
{
    /*
    ...
    {
        SendMessageW( hwnd, WM_WAYLANDDRV_RESIZE_DESKTOP, 0, MAKELPARAM( width, height ) );
    }
    else
    {
